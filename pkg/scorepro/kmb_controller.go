...
type KmbScoreproController struct {
	controllers.Controller
	controllers.BaseBackendController
	service *ScoreproService
}

func NewKmbScoreproController(service *ScoreproService) KmbScoreproController {
	return KmbScoreproController{
...
// @Success 200 {object} controllers.ApiResponse{data=ScoreproResponse}
// @Failure 400 {object} controllers.ApiResponse{}
// @Failure 500 {object} controllers.ApiResponse{}
// @Router /scorepro/kmb/idx [post]
func (c *KmbScoreproController) Scoring(ctx echo.Context) error {
	...
	// Hit Pickle
	dataPickle, status, err, validatePickle, idScore := c.service.HitScoringPickleKmb(request, *modelPickle, transactionID, ctx)
	...
	if *request.CbFound  {
		...
		if isIndosatPrefix {
			...
			if experianDummy != nil {
				...
				// Hit To Experian
				experian, statusExperian, _, err := c.service.HitExperian(param, token,  constant.ScoringExperianKmbOff)
				if err != nil {
					log.Error(err)
					//return c.InternalServerError(ctx, err)
				}
		...
		// Score Combination
		scoreCombination, score, err := c.service.MetricCombintaion(isIndosatPrefix, resp, dataPickle, constant.KmbOffline)
		if err != nil {
			log.Error(err)
			return c.InternalServerError(ctx, err)
		}
